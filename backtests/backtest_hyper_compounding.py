...
print("--> [1/4] Querying 90D dataset from BigQuery...")
...
print("--> [2/4] Computing Causal HMM & Regime Features...")
...
print("--> [3/4] Running Hyper-Compounding Walk-Forward Simulation...")
...
MIN_HMM_BEAR = 0.40          # Aggressively capture short regime
MIN_HMM_BULL = 0.88          # Lock down longs unless ultra-confirmed macro bull
Q_SHORT = 0.245              # Capture full distribution of short breakdowns
Q_LONG = 0.320               # Only top 1% long breakouts allowed
MAX_SLOTS = 2                # Concentrate margin into top 2 alpha leaders
BASE_LEVERAGE = 5.0          # 5.0x gross leverage across slots (2.5x per position)
SHORT_TP_MULT = 3.2          # 3.2x ATR target (captures full liquidation cascade)
SHORT_SL_MULT = 0.85         # Tight 0.85x ATR stop (3.76 : 1 Risk/Reward)
LONG_TP_MULT = 2.5
LONG_SL_MULT = 1.0
MAX_HOLD_BARS = 18
COOLDOWN_BARS = 3
...
for t_idx, ts in enumerate(timestamps):
    ...
    # 2. Sizing & High-Conviction Entries
    ...
    if open_slots > 0:
        ...
        if p_bear >= MIN_HMM_BEAR:
            ...
            for sym, row in cands.iterrows():
                ...
                # Dynamic Leverage scaling up to 3.0x per slot for top-conviction setups
...
print("\n" + "="*55)
print("     HYPER-COMPOUNDING ALPHA PERFORMANCE (90D)")
print("="*55)
