...
# 2. Stable Macro Regime Estimation (Exact Tournament Formulation)
print("--> [2/4] Computing calibrated Causal HMM features...")
...
# Strictly identify states by MBI centroid: 0=Bull (highest MBI), 2=Bear (lowest MBI), 1=Chop
...
# 3. Geometric Compounding Engine (Feature Tournament Specification)
print("--> [3/4] Simulating Tournament Compounding Strategy ($500.00 base)...")
...
for t_idx, ts in enumerate(timestamps):
    ...
    # 2. Geometric Compounding Entries
    ...
    if open_slots > 0:
        ...
        # LONG ENTRIES (Bull Regime + Positive Momentum)
        if p_bull >= MIN_HMM_BULL:
            ...
            for sym, row in cands.iterrows():
                px = float(row["close"])
                atr = float(row["atr"])
                # 1.5x leverage per slot (4.5x total when 3 slots filled)
                notional = (equity * TOTAL_LEVERAGE / MAX_SLOTS)
        ...
        # SHORT ENTRIES (Bear Regime + Negative Momentum)
...
# 4. Analytics
print("--> [4/4] Computing Performance Metrics...")
...
print("\n" + "="*55)
print("     RESTORED FEATURE TOURNAMENT PERFORMANCE (90D)")
print("="*55)
