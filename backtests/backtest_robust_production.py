...
print("--> [1/4] Extracting multi-regime dataset from BigQuery...")
...
print("--> [2/4] Estimating Causal HMM & BTC Trend...")
...
# 3. Robust Walk-Forward Simulation
print("--> [3/4] Running Robust, Regime-Agnostic Simulation ($500.00)...")
...
# Robust, Non-Overfit Parameters
MIN_HMM_REGIME = 0.65        # Symmetric threshold for bull & bear
Q_CONVICTION = 0.250         # Symmetric top-decile CatBoost gate
MAX_SLOTS = 2                # 2 concentrated slots
PORTFOLIO_LEVERAGE = 4.0     # 2.0x leverage per position
TP_MULT = 2.4                # 2.4x ATR take-profit (symmetric)
SL_MULT = 1.0                # 1.0x ATR stop-loss (2.4 : 1 R:R)
MAX_HOLD_BARS = 18
COOLDOWN_BARS = 3
...
for t_idx, ts in enumerate(timestamps):
    ...
    if open_slots > 0:
        ...
        # LONG: Bull Regime + BTC Above EMA20 + Positive Momentum
        if p_bull >= MIN_HMM_REGIME and btc_bull:
            cands = bar_df.loc[eligible_cands]
            cands = cands[(cands["p_model_long"] >= Q_CONVICTION) & (cands["mom_24h"] > 0.0)].sort_values(by="p_model_long", ascending=False).head(open_slots)
            for sym, row in cands.iterrows():
                px = float(row["close"])
                atr = float(row["atr"])
                notional = equity * (PORTFOLIO_LEVERAGE / MAX_SLOTS)
                active_positions[sym] = {
                    "side": "LONG",
                    "entry_price": px,
                    "notional": notional,
                    "tp_px": px + (TP_MULT * atr),
                    "sl_px": px - (SL_MULT * atr),
                    "entry_bar": t_idx
                }

        # SHORT: Bear Regime + Negative Momentum
        elif p_bear >= MIN_HMM_REGIME:
            cands = bar_df.loc[eligible_cands]
            cands = cands[(cands["p_model_short"] >= Q_CONVICTION) & (cands["mom_24h"] < 0.0)].sort_values(by="p_model_short", ascending=False).head(open_slots)
            for sym, row in cands.iterrows():
                px = float(row["close"])
                atr = float(row["atr"])
                notional = equity * (PORTFOLIO_LEVERAGE / MAX_SLOTS)
...
print("\n" + "="*55)
print("     ROBUST PRODUCTION STRATEGY BACKTEST")
print("="*55)
