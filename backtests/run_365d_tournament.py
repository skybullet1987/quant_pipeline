...
print("--> [1/4] Extracting 365D dataset from BigQuery...")
...
print(f"    Loaded {len(df):,} rows across {df['timestamp'].nunique()} 4H bars ({df['timestamp'].min().date()} to {df['timestamp'].max().date()}).")
...
# 3. Full Tournament Geometric Compounding Simulation
print("--> [3/4] Running 1-Year Tournament Compounding Walk-Forward ($500 base)...")
...
# Exact Tournament Compounding Architecture
MIN_HMM_BEAR = 0.40
MIN_HMM_BULL = 0.88
Q_SHORT = 0.245
Q_LONG = 0.320
...
for t_idx, ts in enumerate(timestamps):
    ...
    # Open Positions (Reinvesting 100% of Compounded Capital)
...
print("\n" + "="*55)
print("     FULL 1-YEAR TOURNAMENT COMPOUNDING PERFORMANCE")
print("="*55)
